...
function cc_id_from_page_name ($page) {
  global $wpdb;
  
  $id = $wpdb->get_var ("SELECT ID FROM $wpdb->posts WHERE post_type='page' AND post_name='$page'");
...
function cc_get_attachment ($id) {
	global $wpdb;
 	return $wpdb->get_row("SELECT guid AS uri, post_content AS descr FROM $wpdb->posts WHERE post_parent=$id AND post_type='attachment' ORDER BY ID ASC LIMIT 1;");
}
...
function cc_post_content_process($content) {
  $wrong_br = array("/(<\!--.*)?<(br\/|\/p)>/", "/(<\!--.*)?<p>\&\#8211;>/", "/<\/li><br\s+\/>/");
  $fixed = array("$1", "$1 -->", "</li>");
