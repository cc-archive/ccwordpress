<?php
class w11v_Table_Blogs extends w11v_Table {
	...
	public function move($blog_id, $site_id) {
		$w11v_Table_Sites = new w11v_Table_Sites ( );
		if (! $this->exists ( $blog_id )) {
			return false;
		}
		if (! $w11v_Table_Sites->exists ( $site_id )) {
			return false;
		}
		$site = $w11v_Table_Sites->get ( $site_id );
		...
		$wp_options = new w11v_Table_Options ( $blog_id );
