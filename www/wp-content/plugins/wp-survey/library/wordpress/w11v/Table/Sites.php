<?php
class w11v_Table_Sites extends w11v_Table {
	...
	public function add($domain, $path = '/', $create_blog = true) {
		...
		if (! $this->exists_by_domain ( $domain, $path )) {
			...
			if ($site_id) {
				if ($create_blog) {
					$w11v_Table_Blogs = new w11v_Table_Blogs ( );
					$blog_id = $w11v_Table_Blogs->add ( $site_id );
				}
				...
				$prime_site_meta = new w11v_Table_SiteMeta ( 1 );
				$new_site_meta = new w11v_Table_SiteMeta ( $site_id );
