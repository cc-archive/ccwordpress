<?php
class b11v_Settings extends b11v_Controller_Action {
	...
	public function __construct($application) {
		...
		foreach ( $libraries as $library ) {
			...
			if (file_exists ( $library_file )) {
				$obj = new b11v_Data_INI ( $this->application (), $library_file );
				$ini = $obj->load ()->getArray ();
				$return ['libraries'] [dirname ( $library_file )] = $ini;
			}
		}
		uasort ( $return ['libraries'], array ($this, 'cmp' ) );
		$obj = new b11v_Data_INI ( $this->application (), $plugin_file );
		$ini = $obj->load ()->getArray ();
	...
	public function get_full($html = false) {
		...
		$fs = new b11v_FS($this->application());
