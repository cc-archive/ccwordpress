<?php
abstract class b11v_Controller_Action extends b11v_Base {
	protected function marker($tag, $content) {
		$tagc = b11v_Tag::instance ();
		$matches = $tagc->get ( $tag, $content, true );
		foreach ( ( array ) $matches as $match ) {
			$new = call_user_func ( array ($this, $tag . '_Marker' ), $match );
			$content = str_replace ( $match ['match'], w11v_Values::fixPostInsert ( $new ), $content );
		}
		return $content;
	}
	private $_type = null;
	...
	protected function setView() {
		if (null === $this->view) {
			$this->view = new b11v_View ( $this->application () );
		}
	}
	...
	protected function renderScript($script) {
		...
		if ($script !== false) {
			...
			switch (strtolower ( $pi ['extension'] )) {
				case 'ini' :
					$data = new b11v_Data_INI ( $this->application (), $script );
					$return = $data->load ();
					$return = $return->getArray ();
					break;
				case 'csv' :
					$data = new b11v_Data_CSV ( $this->application (), $script, true );
